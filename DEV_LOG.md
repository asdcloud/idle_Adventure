...
- `runtime.earnedGold/earnedExp`(引擎 onEnemyKilled 累計實際收益)→ shell 每秒取樣、保留 180s 滾動視窗。
- 存檔(autosave + before-quit,統一走 `persist()`)時 `computeOfflineRate()` 快照成 `state.offline`(每秒金幣/經驗,未乘效率)。混合公式:`(視窗收益 + 簡單版×(180-視窗秒))/180` → 滿 3 分=純實際、剛開機=簡單版、中間線性補。
- `settleOffline`:`rate × 離線秒 × 0.7`,上限 8h;離線經驗改走 `gainExp`(回來**立即結算升級**,修掉舊的 `exp +=` 延遲)。SAVE_VERSION 7→8(migrate 補 earnedGold/Exp、offline 預設)。
- 驗證:引擎正確累計 earned*;混合公式 滿3分=200/s純實際、開60s=77/s(1/3實際+2/3簡單)、<1s=簡單版。`tsc` 全綠。
...
### 已知技術債 / 待優化
- [x] ~~離線收益粗估、未實作實際速率取樣~~ → 已改為**近期 3 分鐘實際速率**(不足以簡單版補;2026-06-06,REQ §10.1)。
- [x] ~~離線經驗直接加進 `exp`、未走 `gainExp`~~ → 已改走 `gainExp`,回來立即結算升級(2026-06-06)。
